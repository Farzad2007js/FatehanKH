...
document.getElementsByClassName("b_music")[0].addEventListener("click" , function(){
    ...
    var newTimer=(Number(second)+1);
    second=newTimer.toString();
}
...
function fire(){
    ...
    function win(){
        ...
        if(arr.includes(coor+1)){
            if(coor+1<=64){
                alert("تبریک!!!!! هدف با موفقیت منهدم شد ");
                document.getElementById('img'+(coor+1)).removeAttribute("style");
                var point=Number(document.getElementById('point_a').innerHTML)+1;
                document.getElementById('point_a').innerHTML=point;
            }
        }
            ...
            document.getElementById('div'+(coor+1)).style.backgroundColor="gray";
            }
        }
        if(arr.includes(coor+2)){
            if(coor-1<=64){
                alert("تبریک!!!!! هدف با موفقیت منهدم شد ");
                document.getElementById('img'+(coor-1)).removeAttribute("style");
                var point=Number(document.getElementById('point_a').innerHTML)+1;
